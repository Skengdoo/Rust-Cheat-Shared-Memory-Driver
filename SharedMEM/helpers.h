...
// Shared Memory Vars
const WCHAR g_SharedSectionName[] = L"\\BaseNamedObjects\\SharedMEM";
...
NTSTATUS create_shared_memory()
{
	...
	DbgPrintEx(0, 0, "[Shared MEM] SharedMemory Created\n");
	ExFreePool(Dacl);
...
BOOLEAN call_kernel_function(const char* module_name, void* kernel_function_address, LPCSTR funct_name) 
{
	...
	BYTE instruction[52] = { 0x0 };
	BYTE movInst[2] = { 0x48, 0xBA }; // mov rdx,  
	BYTE jmpInst[2] = { 0xFF, 0xE2 }; // jmp rdx

	BYTE original[] = 
	{
		0x48, 0x8B, 0xC4, 0x48, 0x89, 0x48, 0x08, 0x53, 0x56, 0x57, 0x5B 
	}; 
	...
	memcpy(shell_code + 2, &our_func, sizeof(our_func));
	write_to_read_only_memory(instruction, &shell_code, sizeof(shell_code));

	memcpy(original + 15, &jmpInst, sizeof(jmpInst));
...
BOOLEAN call_kernel_function_ts(void* kernel_function_address)
{
	...
	BYTE dxgk_original[] = { 0x48, 0x8B, 0xC4, 0x48, 0x89, 0x48, 0x08, 0x53, 0x56, 0x57, 0x5B };
	...
	BYTE shell_code_end[] = 
	{
		 0x90, 0x5E, 0x90, 0x5F, 0x90, 0x48, 0xC7, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x90, 0x48, 0x83, 0xC0, 0x19, 0x48, 0xFF, 0xC0, 0x90, 0x48, 0xFF, 0xC8, 0x48, 0x83, 0xE8, 0x19
	};
